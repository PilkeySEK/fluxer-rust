/*
 * Fluxer API
 *
 * API for Fluxer, a free and open source instant messaging and VoIP platform built for friends, groups, and communities.
 *
 * The version of the OpenAPI document: 1.0.0
 * Contact: developers@fluxer.app
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// `PackInviteMetadataResponsePack` : The pack this invite is for
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct PackInviteMetadataResponsePack {
    /// The unique identifier for the pack
    #[serde(rename = "id")]
    pub id: String,
    /// The display name of the pack
    #[serde(rename = "name")]
    pub name: String,
    #[serde(
        rename = "description",
        default,
        with = "::serde_with::rust::double_option",
        skip_serializing_if = "Option::is_none"
    )]
    pub description: Option<Option<String>>,
    /// The type of pack (emoji or sticker)
    #[serde(rename = "type")]
    pub r#type: Type,
    /// The ID of the user who created the pack
    #[serde(rename = "creator_id")]
    pub creator_id: String,
    /// ISO8601 timestamp of when the pack was created
    #[serde(rename = "created_at")]
    pub created_at: String,
    /// ISO8601 timestamp of when the pack was last updated
    #[serde(rename = "updated_at")]
    pub updated_at: String,
    #[serde(rename = "creator")]
    pub creator: Box<models::UserPartialResponse>,
}

impl PackInviteMetadataResponsePack {
    /// The pack this invite is for
    #[must_use] 
    pub fn new(
        id: String,
        name: String,
        r#type: Type,
        creator_id: String,
        created_at: String,
        updated_at: String,
        creator: models::UserPartialResponse,
    ) -> PackInviteMetadataResponsePack {
        PackInviteMetadataResponsePack {
            id,
            name,
            description: None,
            r#type,
            creator_id,
            created_at,
            updated_at,
            creator: Box::new(creator),
        }
    }
}
/// The type of pack (emoji or sticker)
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
#[derive(Default)]
pub enum Type {
    #[serde(rename = "emoji")]
    #[default]
    Emoji,
    #[serde(rename = "sticker")]
    Sticker,
}

