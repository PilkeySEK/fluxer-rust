...
use super::{Endpoint, GLOBAL_LIMIT_PERIOD, RateLimitHeaders};
...
pub async fn runner(
    global_limit: u16,
    mut rx: mpsc::UnboundedReceiver<(Message, Option<super::Predicate>)>,
) {
    ...
    loop {
        tokio::select! {
            ...
            Some((msg, pred)) = rx.recv() => {
                ...
                let is_cancelled = pred.is_some_and(|p| !p(queue.reset.map(|key| super::Bucket {
                    limit: queue.limit,
